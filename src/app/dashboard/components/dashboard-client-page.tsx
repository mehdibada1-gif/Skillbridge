  ...
  useEffect(() => {
    const fetchLearningPaths = async () => {
      ...
      try {
        ...
        if (skills.length > 0) {
          // Use a set to avoid duplicate path fetching
          const uniqueSkills = [...new Set(skills.map(s => s.trim().toLowerCase().replace(/\s+/g, '-')))];
          const pathPromises = uniqueSkills.map(skillId => {
            return getDoc(doc(learningPathsCollection, skillId));
          });
          const pathSnapshots = await Promise.all(pathPromises);
          
          pathSnapshots.forEach((docSnap) => {
            if (docSnap.exists()) {
              const pathData = docSnap.data() as LearningPath;
              // Find the original skill name to use as a key
              const originalSkill = skills.find(s => s.trim().toLowerCase().replace(/\s+/g, '-') === docSnap.id);
              paths.set(originalSkill || pathData.name, pathData.modules);
            }
          });
        }
        
        // If no skill-based paths were found, fetch the default one
        if (paths.size === 0) {
